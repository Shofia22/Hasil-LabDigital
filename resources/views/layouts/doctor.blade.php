...
@php($notesBadge = \App\Models\DoctorNote::where('doctor_id', Auth::id())->count())
<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="utf-8">
        ...
        <aside class="col-span-12 md:col-span-3 lg:col-span-2 bg-white border-r border-gray-200 flex flex-col">
            <div class="px-6 py-6 flex items-center space-x-3">
                ...
                <div>
                    ...
                    <div class="text-xs text-gray-500">Doctor Panel</div>
                </div>
            </div>
            <nav class="px-3 space-y-1">
                <a href="{{ route('doctor.dashboard') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium {{ request()->routeIs('doctor.dashboard') ? 'bg-emerald-600 text-white' : 'text-gray-700 hover:bg-gray-100' }}">
                ...
                <a href="{{ route('doctor.lab-results') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium {{ request()->routeIs('doctor.lab-results') ? 'bg-emerald-50 text-emerald-700' : 'text-gray-700 hover:bg-gray-100' }}">
                ...
                <a href="{{ route('doctor.notes') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium {{ request()->routeIs('doctor.notes') ? 'bg-emerald-50 text-emerald-700' : 'text-gray-700 hover:bg-gray-100' }}">
                    <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21H5a2 2 0 01-2-2V5a2 2 0 012-2h9l7 7v9a2 2 0 01-2 2z"/><path d="M17 21v-8h-6"/></svg>
                    Catatan Dokter
                    <span class="ml-auto inline-flex items-center justify-center min-w-[1.25rem] h-5 px-1.5 text-[11px] rounded-full {{ request()->routeIs('doctor.notes') ? 'bg-emerald-600 text-white' : 'bg-gray-200 text-gray-700' }}">{{ $notesBadge }}</span>
                </a>
            </nav>
